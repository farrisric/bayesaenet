...
root = pyrootutils.setup_root(
    search_from=__file__, indicator=[".git", "pyproject.toml"],
...
CONFIGS = {
    ("lrt", "low"):  dict(batch_size=256, mc=1, fit_context="lrt", guide="normal",
                          train_fraction=0.2),
    ("rad", "low"):  dict(batch_size=512, mc=2, fit_context=None, guide="radial",
                          train_fraction=0.2),
    ("de",  "low"):  dict(batch_size=128, train_fraction=0.2),
    ("lrt", "high"): dict(batch_size=128, mc=1, fit_context="lrt", guide="normal",
                          train_fraction=1.0),
    ("rad", "high"): dict(batch_size=256, mc=1, fit_context=None, guide="radial",
                          train_fraction=1.0),
...
EPOCHS = {
    ("lrt", "low"): (6994, 1128), ("rad", "low"): (2533, 385), ("de", "low"): (1777, 237),
    ("lrt", "high"): (2641, 156), ("rad", "high"): (4254, 263), ("de", "high"): (4185, 149),
}

DE_MEMBERS = 5  # reported DE is a 5-member ensemble (pool of 10 trained, 5 used)
ORDER = [("lrt", "low"), ("rad", "low"), ("de", "low"),
         ("lrt", "high"), ("rad", "high"), ("de", "high")]
...
def build_datamodule(data_dir, batch_size, train_fraction):
    ...
    return AenetDataModule(
        data_dir=str(data_dir), device="cpu", batch_size=batch_size,
        test_split=0.1, valid_split=0.1, train_fraction=train_fraction,
        name="TiO2_Forces",
    )
...
def build_net(dm):
    ...
    return NetAtom(
        input_size=dm.input_size, hidden_size=dm.hidden_size,
        species=dm.species, active_names=dm.active_names, alpha=0.1,
        device="cpu", e_scaling=dm.e_scaling, e_shift=dm.e_shift,
    )
...
def build_model(model, dm, cfg):
    ...
    return BNN_Forces(
        net=net, lr=1e-4, pretrain_epochs=0, mc_samples_train=cfg["mc"],
        mc_samples_eval=20, dataset_size=dm.train_size,
        fit_context=cfg["fit_context"], prior_loc=0, prior_scale=0.3,
        guide=cfg["guide"], q_scale=2e-4, obs_scale=0.5, scale_force=0.1,
        learn_noise=True,
    )
...
def time_config(model, regime, data_dir, device, epochs, warmup):
    ...
    trainer = Trainer(
        max_epochs=epochs, accelerator=device, devices=1, logger=False,
        enable_checkpointing=False, enable_progress_bar=False,
        num_sanity_val_steps=0, precision=32, callbacks=[timer],
    )
...
def main():
    ...
    p.add_argument("--data-dir",
                   default=str(root / "data" / "TiO" / "train_forces_local.in"))
    p.add_argument("--epochs", type=int, default=60)
    p.add_argument("--warmup", type=int, default=10)
    p.add_argument("--output",
                   default=str(Path(__file__).parent / "train_time_5090_byregime.csv"))
    args = p.parse_args()
    ...
    for model, regime in ORDER:
        cfg = CONFIGS[(model, regime)]
        mean_s, std_s = time_config(model, regime, args.data_dir, args.device,
        ...
        print(f"{regime:>4} {model.upper():<4} batch={cfg['batch_size']:>4} "
        ...
        rows.append({
            ...
            "batch_size": cfg["batch_size"], "mc": cfg.get("mc", "-"),
            "train_fraction": cfg["train_fraction"],
            "per_epoch_s": round(mean_s, 4), "per_epoch_s_std": round(std_s, 4),
            "epochs": ep_mean, "epochs_std": ep_std,
            "total_h": round(total_h, 3), "total_h_std": round(total_h_std, 3),
        })
