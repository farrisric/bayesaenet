...
for run in runs:
    size = run.split('/')[-1].split('_')[1]
    best = run.split('/')[-1].split('_')[-2]
    if best != 'best':
        continue
    if size != 'small':
        continue
    ...
    # Prepare figure
    os.makedirs(f'{cwd}/figs', exist_ok=True)
    ...
    fig.tight_layout()
    fig.savefig(f'{cwd}/figs/{run.split("/")[-1]}_events.png', bbox_inches='tight')
    plt.close(fig)
