...
for size in sizes:
    for run in glob.glob(f'{logs}/DE/{method}_pred_{size}*/runs/2025-03*'):
        print(run)
        if size == 20:
            size = 'small'
        else:
            size = 'big'
        run_name = run.split('/')[-3]    
        ...
        metrics = get_metrics(torch.tensor(y_true_all), torch.tensor(y_pred_all), torch.tensor(y_std_all))
        metrics['Overlap'] = analyze_uncertainty_and_error(y_true_all, y_pred_all, y_std_all)
        metrics['Method'] = method
        metrics['Size'] = size
        metrics['Run'] = run_name.split('_')[-1]
        metrics['Split'] = split
        all_metrics_df[split].append(metrics)
