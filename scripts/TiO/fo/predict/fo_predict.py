...
def get_metrics(y_true, y_pred, std):
    ...
    return {'mse': mse, 'rmse': rmse, 'maxerr' : max_error, 'rmsce': rmsce, 'sharp': sharp, 'ece': ece, 'nll': nll}

parquets = sorted(glob.glob('/home/g15farris/bin/bayesaenet/bnn_aenet/logs/fo_pred*/**/*parquet', recursive=True))

outfile =  open(f'/home/g15farris/bin/bayesaenet/scripts/TiO/fo/predict/metrics.txt', 'w')

outfile.write('Dataset Perc\tR2 score\tRMSE\tMAE\tMAX error\n')
...
for i, parquet in enumerate(parquets):
    ...
    textstr = '\n'.join((
        f'Dataset Perc: {train_percentage}%',
        f'R2 score: {r2:.3f}',
        f'RMSE: {rmse:.3f} eV',
        f'MAE: {mae:.3f} eV',
    ...
    text = '\t'.join([train_percentage, str(r2), str(rmse), str(mae), str(max_error)])
    outfile.write(text)
    outfile.write('\n')
    ...
    plt.savefig(f'/home/g15farris/bin/bayesaenet/scripts/TiO/fo/predict/{name_file}_{train_percentage}.png', dpi=300)
    plt.close(fig)
    ...
    plt.savefig(f'/home/g15farris/bin/bayesaenet/scripts/TiO/fo/predict/{name_file}_{train_percentage}_uct.png', dpi=300)
