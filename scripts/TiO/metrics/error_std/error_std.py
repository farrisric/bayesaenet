...
# Define R² computation and regression line function
...
sizes = ['20', '100']
out_dir = "/home/g15farris/bin/bayesaenet/scripts/TiO/metrics/error_std"
...
for model in models:
    ...
    for size_idx, size in enumerate(sizes):
        ...
        model_runs = sorted(model_runs)[:5]  # Take first 5 runs
        ...
        for run_name in model_runs:
            ...
            lim = max(y_std.max(), abs_error.max())
            ...
            ax.set_title(f"{model.upper()}: {size}% Run {plot_idx % 5 + 1}\n$R^2$ = {r2:.2f}", fontsize=10)
            ...
            inset_ax.set_ylim(-0.005, zoom_lim)
    ...
    fig.savefig(f"{out_dir}/{model}_scatter_grid_paper.png", dpi=200, bbox_inches="tight")
    plt.close()

print("All model grids saved to:", out_dir)
