...
for perc in list(range(10, 81, 10)):
    # Define paths
    if perc == 60:
        continue
    input_path = f'/home/g15farris/bin/bayesaenet/bnn_aenet/logs/predict_lrt_{perc}perc/runs/*/'
    output_path = '/home/g15farris/bin/bayesaenet/results/PdO/bnn_lrt/metrics'
    ...
    for parquet in parquets:
        try:
            ...
            y_true = rs['true'].to_numpy()#/rs['n_atoms']
            y_pred = rs['preds'].to_numpy()#/rs['n_atoms']
            y_std = rs['stds'].to_numpy()/rs['n_atoms'].to_numpy()
    ...
    ax[0].scatter(y_true, y_pred, color='tab:blue', alpha=0.7, label='Predictions')
