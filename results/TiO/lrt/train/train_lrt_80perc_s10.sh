#!/bin/bash
#$ -N fin_s10_train_lrt_80perc_TiO
#$ -pe smp* 1
#$ -q iqtc09.q
#$ -S /bin/bash
#$ -cwd
...
python bnn_aenet/tasks/train.py \
    ...
    task_name=final_TiO_train_lrt_80_sigmaprior10 \
    datamodule=TiO \
    datamodule.test_split=0.1 \
    datamodule.valid_split=0.1 \
    datamodule.batch_size=32 \
    model.lr=0.000992657613505169 \
    model.pretrain_epochs=5 \
    model.mc_samples_train=1 \
    model.mc_samples_eval=20 \
    model.prior_loc=0 \
    model.prior_scale=10.00 \
    model.q_scale=0.0007705241148640707 \
    model.obs_scale=0.9906127694463113
