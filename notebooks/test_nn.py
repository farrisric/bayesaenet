...
datamodele = AenetDataModule(data_dir='../data/IrO/train.in',batch_size=100,test_split=0.1,valid_split=0.1)
...
model_kwargs = {'net': net,
        ...
        'guide': 'radial', 
        'pretrain_epochs': 0, 
        'lr': 0.000998500562121643, 
        'mc_samples_train': 2, 
        'prior_scale': 0.06806796027857094, 
        'q_scale': 0.0036974615971953873, 
        'obs_scale': 0.7275588065675455
        }
...
ckpt_path = '/home/riccardo/bin/repos/aenet-bnn/notebooks/lightning_logs/version_42/checkpoints/epoch=141-step=23004.ckpt'

model = BNN(**model_kwargs)
trainer = L.Trainer(max_epochs = 9000)
...
for name, dataset in zip(names, datasets):
    ...
    y_pred, y_std, y_true = y_pred[:40], y_std[:40], y_true[:40]
