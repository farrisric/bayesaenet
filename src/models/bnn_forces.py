...
class BNN(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        y = batch[1]
        group_N_atom =  batch[4]
        group_descrp =  batch[0]
        group_sfderiv_i = batch[6]
        group_sfderiv_j = batch[7]
        group_indices_F = batch[8]
        grp_indices_F_i = batch[9]
        logic_reduce = batch[2]
        group_forces = batch[5]
        input_size =  input_size
        max_nnb = max_nnb
        
        x = group_descrp, group_sfderiv_i, group_sfderiv_j, group_indices_F, grp_indices_F_i, logic_reduce, input_size, max_nnb
