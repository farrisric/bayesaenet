...
from .aenet.prepare_batches import read_list_structures, select_batch_size, select_batches
...
class AenetDataModule(L.LightningDataModule):
    ...
    def load_db(self):
        ...
        train_set_size, N_batch_train, N_batch_valid, N_batch_test = select_batch_size(
            self.tin, self.list_structures_energy, self.list_structures_forces
        )
        ...
        self.grouped_train_data = GroupedDataset(
            train_energy_data,
            train_forces_data,
            memory_mode=self.tin.memory_mode,
            device=self.device,
            dataname="train",
        )
        self.grouped_valid_data = GroupedDataset(
            valid_energy_data,
            valid_forces_data,
            memory_mode=self.tin.memory_mode,
            device=self.device,
        ...
        self.grouped_test_data = GroupedDataset(
            test_energy_data,
            test_forces_data,
            memory_mode=self.tin.memory_mode,
            device=self.device,
        ...
        self.grouped_all_data = GroupedDataset(
            all_energy_data,
            None,
            memory_mode=self.tin.memory_mode,
            device=self.device,
