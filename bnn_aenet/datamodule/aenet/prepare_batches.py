...
def read_list_structures(tin):
    ...
    """
    if tin.train_forces:
        list_structures_forces, list_structures_energy, list_removed, max_nnb, tin = (
            read_train_forces_together(tin)
        )
    else:
...
def split_database(dataset_size, valid_split, test_split, seed=42, train_fraction=1.0):
    """
...
def select_batches(
    ...
    if len(list_structures_energy) != 0:
        dataset_energy = StructureDataset(
            list_structures_energy, tin.sys_species, tin.networks_param["input_size"], max_nnb
        )
        ...
        train_fraction = getattr(tin, "train_fraction", 1.0)
        train_sampler_E, valid_sampler_E, test_sampler_E = split_database(
            dataset_energy_size, tin.valid_split, tin.test_split, train_fraction=train_fraction
        )

        train_energy_data = PrepDataloader(
            dataset=dataset_energy,
            train_forces=False,
        ...
        valid_energy_data = PrepDataloader(
            dataset=dataset_energy,
            train_forces=False,
            N_batch=N_batch_valid,
            sampler=valid_sampler_E,
            memory_mode=tin.memory_mode,
            device=device,
            dataname="valid_energy",
        )
        test_energy_data = PrepDataloader(
            dataset=dataset_energy,
            train_forces=False,
            N_batch=N_batch_test,
            sampler=test_sampler_E,
            memory_mode=tin.memory_mode,
            device=device,
            dataname="test_energy",
        )
        all_energy_data = PrepDataloader(
            dataset=dataset_energy,
            train_forces=False,
            N_batch=len(dataset_energy),
            sampler=range(len(dataset_energy)),
            memory_mode=tin.memory_mode,
            device=device,
            dataname="all_energy",
        )
    ...
    if len(list_structures_forces) != 0:
        dataset_forces = StructureDataset(
            list_structures_forces, tin.sys_species, tin.networks_param["input_size"], max_nnb
        )
        ...
        train_fraction = getattr(tin, "train_fraction", 1.0)
        train_sampler_F, valid_sampler_F, test_sampler_F = split_database(
            dataset_forces_size, tin.valid_split, tin.test_split, train_fraction=train_fraction
        )

        train_forces_data = PrepDataloader(
            dataset=dataset_forces,
            train_forces=True,
            N_batch=N_batch_train,
            sampler=train_sampler_F,
            memory_mode=tin.memory_mode,
            device=device,
            dataname="train_forces",
        )
        valid_forces_data = PrepDataloader(
            dataset=dataset_forces,
            train_forces=True,
            N_batch=N_batch_valid,
            sampler=valid_sampler_F,
            memory_mode=tin.memory_mode,
            device=device,
            dataname="valid_forces",
        )
        test_forces_data = PrepDataloader(
            dataset=dataset_forces,
            train_forces=True,
            N_batch=N_batch_test,
            sampler=test_sampler_F,
            memory_mode=tin.memory_mode,
            device=device,
            dataname="test_forces",
        )
