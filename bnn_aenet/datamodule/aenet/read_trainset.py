...
def read_train_forces_together(tin):
    ...
    with open(trainfile, "r") as tf, open(forcesfile, "rb") as tff:
        ...
        # Header
        N_species, N_struc, species_index, E_atomic, normalized, E_scaling, E_shift = (
            tf_read_header(tf, sys_species)
        )
        _ = tff_read_header(tff)

        # Footer (Fingerprint Setup information)
        natomstot, E_avg, E_min, E_max, setup_params, input_size = tf_read_footer(
            tf, N_species, species_index
        )

        trainset_params = TrainSetParameter(
            trainfile,
            normalized,
            E_scaling,
            E_shift,
        ...
        for istruc in range(N_struc):
            name, E, E_atomic_structure, species, coords, forces, descriptors = tf_read_struc_info(
                tf, species_index, E_atomic
            )
            train_forces_struc, max_nnb, list_nblist, list_sfderiv_i, list_sfderiv_j = (
                tff_read_struc_info_grads(tff, species_index, max_nnb)
            )
        ...
        trainset_params.E_avg = E_max_min_avg[2] / (
            len(list_struct_forces) + len(list_struct_energy)
        )
