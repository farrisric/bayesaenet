...
if tin.load_batches:
    ...
    if tin.verbose:
        io_trainingset_information_done(tin, tin.trainset_params, N_struc_E, N_struc_F, N_removed)


else:
    # Read datasets
    if tin.verbose:
        io_trainingset_information()
    list_structures_energy, list_structures_forces, list_removed, max_nnb, tin = (
        read_list_structures(tin)
    )
    ...
    train_set_size, N_batch_train, N_batch_valid, N_batch_test = select_batch_size(
        tin, list_structures_energy, list_structures_forces
    )
    ...
    if tin.verbose:
        io_prepare_batches_done(tin, train_energy_data, train_forces_data)
...
grouped_train_data = GroupedDataset(
    train_energy_data,
    train_forces_data,
...
grouped_valid_data = GroupedDataset(
    valid_energy_data,
    valid_forces_data,
    memory_mode=tin.memory_mode,
    device=device,
...
if tin.verbose:
    io_network_initialize(tin)

model = NetAtom(
    tin.networks_param["input_size"],
    tin.networks_param["hidden_size"],
    tin.sys_species,
    tin.networks_param["activations"],
    tin.alpha,
    device,
)
model.to(device)
...
grouped_train_loader = DataLoader(
    grouped_train_data, batch_size=1, shuffle=False, collate_fn=custom_collate, num_workers=0
)
grouped_valid_loader = DataLoader(
    grouped_valid_data, batch_size=1, shuffle=False, collate_fn=custom_collate, num_workers=0
)


# 5. Train
if tin.verbose:
    io_train_details(tin, device)
if tin.verbose:
    io_train_start()
t = time.time()
iter_error_trn = []
...
for epoch in range(tin.epoch_size):
    ...
    train_error, train_E_error, train_F_error = step_train_any(
        model, grouped_train_loader, E_scaling, tin.networks_param["input_size"], max_nnb
    )
    # iter_error_trn.append([epoch, train_error, train_E_error, train_F_error])

    if epoch % tin.epoch_write == 0:
        valid_error, valid_E_error, valid_F_error = step_valid_any(
            model, grouped_valid_loader, E_scaling, tin.networks_param["input_size"], max_nnb
        )
        io_train_step(
            epoch,
            train_error,
