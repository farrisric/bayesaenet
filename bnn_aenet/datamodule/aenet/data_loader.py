...
class PrepDataloader(object):
    ...
    def __getitem__(self, index):
        ...
        if self.train_forces:
            return (
                self.group_descrp[index],
                self.group_energy[index],
                self.logic_tensor_reduce[index],
                self.index_from_database[index],
                self.group_N_atom[index],
                self.group_forces[index],
                self.group_sfderiv_i[index],
                self.group_sfderiv_j[index],
                self.group_indices_F[index],
    ...
    def initialize(self):
        ...
        self.batch_names = [
            "./tmp_batches/" + self.dataname + "batch_energy{:}.pkl".format(ibatch)
            for ibatch in range(self.N_batch)
        ]
    ...
    def batch_data_cpu_to_gpu(self, ibatch, device):
        ...
        if self.train_forces:
            ...
            for iesp in range(len(self.group_descrp[ibatch])):
                ...
                self.logic_tensor_reduce[ibatch][iesp] = self.logic_tensor_reduce[ibatch][iesp].to(
                    device
                )
    ...
    def prepare_batch_forces(self, ibatch):
        ...
        for istruc in range(index[0], index[1]):
            ...
            for iesp in range(self.dataset.N_species):
                group_N_atom_iesp[iesp] = (
                    group_N_atom_iesp[iesp] + self.dataset[index_struc].N_ions[iesp]
                )
        ...
        index_from_database = []
        group_descrp = [
            torch.empty(0, self.dataset.input_size[iesp]) for iesp in range(self.dataset.N_species)
        ]
        group_forces = torch.empty(0, 3)
        group_sfderiv_i = [
            torch.empty(0, self.dataset.input_size[iesp], 3)
            for iesp in range(self.dataset.N_species)
        ]
        group_sfderiv_j = [
            torch.empty(0, self.dataset.max_nnb, self.dataset.input_size[iesp], 3)
            for iesp in range(self.dataset.N_species)
        ]
        ...
        group_nblist = [
            torch.empty(0, self.dataset.max_nnb, dtype=int)
            for iesp in range(self.dataset.N_species)
        ]
        ...
        for istruc in range(index[0], index[1]):
            ...
            index_from_database.append(
                [self.dataset[index_struc].name, self.dataset[index_struc].E_atomic_structure]
            )

            ind_start_iesp = 0

            for iesp in range(self.dataset.N_species):
                group_descrp[iesp] = torch.cat(
                    (group_descrp[iesp], self.dataset[index_struc].descriptor[iesp])
                )
                group_nblist[iesp] = torch.cat(
                    (
                        group_nblist[iesp],
                        self.dataset[index_struc].list_nblist[iesp] + ind_start_struc,
                    )
                )

                commands_i[iesp] += "self.dataset[{:}].list_sfderiv_i[{:}], ".format(
                    index_struc, iesp
                )
                commands_j[iesp] += "self.dataset[{:}].list_sfderiv_j[{:}], ".format(
                    index_struc, iesp
                )
                ...
                group_indices_F_i[ind2 : ind2 + N_ions_istruc] = torch.arange(
                    ind1, ind1 + N_ions_istruc
                )
    ...
    def prepare_batches(self):
        ...
        group_descrp = [
            [
                torch.empty(0, self.dataset.input_size[iesp])
                for iesp in range(self.dataset.N_species)
            ]
            for ibatch in range(self.N_batch)
        ]
        ...
        for ibatch in range(self.N_batch):
            ...
            for istruc in range(index[0], index[1]):
                ...
                index_from_database[ibatch].append(
                    [self.dataset[index_struc].name, self.dataset[index_struc].E_atomic_structure]
                )
                ...
                for iesp in range(self.dataset.N_species):
                    group_descrp[ibatch][iesp] = torch.cat(
                        (group_descrp[ibatch][iesp], self.dataset[index_struc].descriptor[iesp])
                    )
