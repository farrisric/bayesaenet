...
def init_optimizer(tin, model):
    if tin.method == "adam":
        model.optimizer = torch.optim.Adam(
            model.parameters(), lr=tin.lr, weight_decay=tin.regularization
        )

    if tin.method == "adadelta":
        model.optimizer = torch.optim.Adadelta(
            model.parameters(), lr=tin.lr, weight_decay=tin.regularization
        )

    if tin.method == "adagrad":
        model.optimizer = torch.optim.Adagrad(
            model.parameters(), lr=tin.lr, weight_decay=tin.regularization
        )
    ...
    if tin.method == "adamax":
        model.optimizer = torch.optim.Adamax(
            model.parameters(), lr=tin.lr, weight_decay=tin.regularization
        )
...
def init_transfer(tin, trainset_params, model):
    ...
    E_scaling, E_shift, E_atomic = (
        trainset_params.E_scaling,
        trainset_params.E_shift,
        trainset_params.E_atomic,
    )
    ...
    if os.path.exists("./model.transfered.restart"):
        ...
        for iesp in range(tin.N_species):
            with open(tin.networks_param["names"][iesp] + ".transfered.ascii", "r") as f:
                ...
                neval_i, sfval_min_i, sfval_max_i, sfval_avg_i, sfval_cov_i = read_setup_iesp(
                    tin.sf_setups_param, iesp, f
                )
