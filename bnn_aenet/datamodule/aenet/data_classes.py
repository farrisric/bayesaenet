...
class Structure(object):
    ...
    def __init__(
        ...
        self.species = []
        self.descriptor = [
            [[] for iat in range(self.N_ions[iesp])] for iesp in range(self.N_species)
        ]
        ...
        if train_forces:
            ...
            self.list_sfderiv_i = [
                torch.zeros(self.N_ions[iesp], input_size[iesp], 3)
                for iesp in range(self.N_species)
            ]
            self.list_sfderiv_j = [
                torch.zeros(self.N_ions[iesp], self.max_nb_struc, input_size[iesp], 3)
                for iesp in range(self.N_species)
            ]
            ...
            for iesp in range(self.N_species):
                for iat in range(self.N_atom):
                    if species[iat] == iesp:
                        ...
                        self.list_sfderiv_j[iesp][cont_iesp[iesp]][:nnb, :, :] = list_sfderiv_j[
                            iat
                        ]
        ...
        if self.train_forces:
            for iesp in range(self.N_species):
                if self.N_ions[iesp] == 0:
                    ...
                    self.list_sfderiv_j[iesp] = torch.zeros(
                        self.N_ions[iesp], self.max_nb_struc, input_size[iesp], 3
                    )

            # Add trailing zeros
            aux_nblist = [
                torch.ones(self.N_ions[iesp], max_nnb, dtype=int) * -1000000000
                for iesp in range(self.N_species)
            ]
            aux_sfderiv_j = [
                torch.zeros(self.N_ions[iesp], max_nnb, input_size[iesp], 3)
                for iesp in range(self.N_species)
            ]
            ...
            for iesp in range(self.N_species):
                for iat in range(self.N_ions[iesp]):
                    ...
                    aux_sfderiv_j[iesp][iat, :nnb, :, :] = self.list_sfderiv_j[iesp][iat][
                        :nnb, :, :
                    ]
            ...
            for iat in range(N_atom_total):
                if self.order[iat] != iat:
                    for iesp in range(self.N_species):
                        sorted_nblist[iesp] = torch.where(
                            aux_nblist[iesp] == iat, self.order[iat], sorted_nblist[iesp]
                        )
