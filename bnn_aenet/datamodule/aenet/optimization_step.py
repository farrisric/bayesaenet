...
def step_train_any(net, train_data, E_scaling, input_size, max_nnb):
    ...
    for i_batch, data_batch in enumerate(train_data):
        ...
        if train_data.dataset.train_forces:
            l1_E, l1_F, N_data1_E, N_data1_F = net.get_loss_RMSE_F(
                data_batch[0][1],
                data_batch[0][4],
                data_batch[0][0],
                data_batch[0][6],
        ...
        if train_data.dataset.train_energy:
            l2_E, N_data2_E = net.get_loss_RMSE(
                data_batch[0][10], data_batch[0][11], data_batch[0][12], data_batch[0][14]
            )
            ...
            with torch.no_grad():
                if any_data.dataset.train_forces:
                    ...
                    for i_struc in range(len(group_energy)):
                        ...
                        f.write(
                            fmt.format(
                                int(N_atoms), E_ref, E_ann, E_ref / N_atoms, E_ann / N_atoms, name
                            )
                        )
...
def save_forces_any(net, any_data, E_scaling, input_size, max_nnb, dataset="train"):
    ...
    with open("forces." + dataset, "w") as f:
        ...
        for i_batch, data_batch in enumerate(any_data):
            ...
            # If forces
            if any_data.dataset.train_forces:
                list_E_ann, list_F_ann = net.forward_F(
                    data_batch[0][0],
                    data_batch[0][6],
                    data_batch[0][7],
                    data_batch[0][8],
                ...
                for i_struc in range(len(list_E_ann)):
                    ...
                    for i_atom in range(N_atoms):
                        ...
                        f.write(
                            fmt.format(i_atom, l_F, *F_struc_ref[i_atom], *F_struc_ann[i_atom])
                        )
