        ...
        for struc in self.list_struc:
            for iesp in range(self.N_species):
                ...
                if struc.train_forces:
                    struc.list_sfderiv_i[iesp] = torch.einsum(
                        "ijk,j->ijk", struc.list_sfderiv_i[iesp], scale[iesp]
                    )
                    struc.list_sfderiv_j[iesp] = torch.einsum(
                        "ijkl,k->ijkl", struc.list_sfderiv_j[iesp], scale[iesp]
                    )
...
class GroupedDataset(object):
    ...
    def __init__(
        self, energy_data, forces_data, generate=True, memory_mode="cpu", device="cpu", dataname=""
    ):
    ...
    def __getitem__(self, index):
        ...
        if self.memory_mode == "cpu" or self.memory_mode == "gpu":
            data = [
                self.F_group_descrp[index],
                self.F_group_energy[index],
                self.F_logic_tensor_reduce[index],
                self.F_index_from_database[index],
                self.F_group_N_atom[index],
                self.F_group_forces[index],
                self.F_group_sfderiv_i[index],
                self.F_group_sfderiv_j[index],
                self.F_group_indices_F[index],
                self.F_group_indices_F_i[index],
                self.E_group_descrp[index],
                self.E_group_energy[index],
                self.E_logic_tensor_reduce[index],
                self.E_index_from_database[index],
                self.E_group_N_atom[index],
            ]
    ...
    def save_batches(
        ...
        save = {
            "E_batch_names": self.E_batch_names,
