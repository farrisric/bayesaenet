...
def io_input_reading(tin):
    ...
    if tin.train_forces:
        ...
        if tin.max_forces:
            io_print("        - maxforces:   " + str(tin.max_forces))
        io_print("")
...
def io_network_initialize(tin):
    ...
    for iesp in range(tin.N_species):
        ...
        io_print("")
        io_print(
            "Number of layers: {:4d}".format(len(tin.networks_param["hidden_size"][iesp]) + 2)
        )
        io_print("")
        ...
        for ilayer in range(len(tin.networks_param["hidden_size"][iesp])):
            io_print(
                "   {:2d} : {:6d}   {:}".format(
                    ilayer + 2,
                    tin.networks_param["hidden_size"][iesp][ilayer],
                    tin.networks_param["activations"][iesp][ilayer],
                )
            )
...
def io_save_networks(tin):
    ...
    for iesp in range(tin.N_species):
        io_print(
            "Saving the {:} network to file : {:}".format(
                tin.sys_species[iesp], tin.networks_param["names"][iesp] + ".ascii"
            )
        )
