...
from tyxe import VariationalBNN, guides, likelihoods, priors
...
from .utils import get_rmse_atom, param_store_to, remove_dict_entry_startswith, weights_init
...
class BNN(L.LightningModule):
    ...
    def on_fit_start(self) -> None:
        ...
        self.optimizer = pyro.optim.ClippedAdam(
            {"lr": self.hparams.lr, "betas": [0.95, 0.999], "clip_norm": clip_norm}
        )
        self.loss = (
    ...
    def training_step(self, batch: List[torch.Tensor], batch_idx: int) -> None:
            ...
            with torch.no_grad():
                for name, val in pyro.get_param_store().items():
                    if torch.isnan(val).any():
                        warnings.warn(
                            f"NaN detected in Pyro param '{name}' after SVI step, clamping."
                        )
                        val.data = torch.nan_to_num(val.data, nan=0.0)

            loc, scale = self.bnn.predict(
                x[0], x[1], num_predictions=self.hparams.mc_samples_train
            )
        ...
        self.log(
            "rmse/train", rmse, on_step=False, prog_bar=True, on_epoch=True, batch_size=len(y)
        )
    ...
    def predict_step(
        ...
        output = self.bnn.predict(
            x[0], x[1], num_predictions=self.hparams.mc_samples_eval, aggregate=False
        )
