...
class NetAtom(nn.Module):
    ...
    def __init__(self, input_sizes, hidden_sizes, species, activation_names, alpha, device, energy_scaling, energy_shift):
        """
        Initialize the network.

        Args:
            input_sizes (list): List of input sizes for each species.
            hidden_sizes (list): List of hidden layer sizes for each species.
            species (list): List of species in the dataset.
            activation_names (list): List of activation function names for each species.
            alpha (float): Regularization parameter.
            device (torch.device): Device to run the model on (e.g., "cpu" or "cuda").
            energy_scaling (float): Scaling factor for energy.
            energy_shift (float): Shift factor for energy.
        """
        ...
        self.networks = nn.ModuleList([
            self._build_network(input_size, hidden_size, activation_names[i])
            for i, (input_size, hidden_size) in enumerate(zip(input_sizes, hidden_sizes))
        ])

    def _build_network(self, input_size, hidden_size, activation_names):
        """
        ...
        Args:
            ...
            activation_names (list): Names of the activation functions for each layer.
        ...
        for i, (n_in, n_out) in enumerate(zip([input_size] + hidden_size[:-1], hidden_size)):
            ...
            layers[f"Activation_Layer_{i+1}"] = self.activations[activation_names[i]]
