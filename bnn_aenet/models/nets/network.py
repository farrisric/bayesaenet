...
class NetAtom(nn.Module):
    ...
    def __init__(
        self, input_size, hidden_size, species, active_names, alpha, device, e_scaling, e_shift
    ):
        ...
        for i in range(len(species)):
            ...
            for j in range(len(hidden_size[i])):
                ...
                if act_name == "linear":
                    ...
                    raise ValueError(
                        f"Unknown activation: {act_name}. Supported: linear, tanh, sigmoid, relu, softplus, elu, gelu, silu/swish"
                    )
    ...
    def forward_F(
        ...
        E_atomic_ann = [
            0 for i in range(len(self.species))
        ]  # torch.empty((0,1), requires_grad=True)
        ...
        for iesp in range(len(self.species)):
            ...
            aux_F_j = torch.cat(
                (aux_F_j, torch.einsum("ik,ijkl->ijl", aux_dE_dG, group_sfderiv_j[iesp]))
            )
            aux_F_i = torch.cat(
                (aux_F_i, torch.einsum("ik,ikl->il", aux_dE_dG, group_sfderiv_i[iesp]))
            )
        ...
        F_ann = -torch.index_select(aux_F_i, 0, grp_indices_F_i) - torch.sum(
            torch.index_select(aux_F_flat, 0, flatt_indices).reshape(aux_shape), dim=1
        )

        list_E_ann = torch.zeros((len(logic_reduce[0])), device=device)
        for iesp in range(len(self.species)):
            list_E_ann = list_E_ann + torch.einsum(
                "ij,ki->k", E_atomic_ann[iesp], logic_reduce[iesp]
            )
    ...
    def get_loss_RMSE_F(
        ...
        list_E_ann, list_F_ann = self.forward_F(
            group_descrp,
            group_sfderiv_i,
            group_sfderiv_j,
            group_indices_F,
