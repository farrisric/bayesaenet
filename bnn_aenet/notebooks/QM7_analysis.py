...
def compute_metrics(y_true, y_pred, y_std=None):
    ...
    if y_std is not None:
        ...
        metrics.update({"overlap": overlap, "sharp": sharpness, "nll": nll})
...
def analyze_method(method_name, pred_dir):
    ...
    for run_dir in run_dirs:
        ...
        try:
            ...
            if has_uq:
                print(
                    f"  ✓ {run_name:30s} MAE={metrics['mae']:.3f}, RMSE={metrics['rmse']:.3f}, "
    ...
    if len(all_results) > 1:
        ...
        for method, df in all_results.items():
            ...
            if "overlap" in df.columns:
                print(
                    f"       UQ: Overlap={df['overlap'].mean():.1f}%, Sharpness={df['sharp'].mean():.4f}"
                )
